...
static void GrowPool(List *lst) {
    ...
    size_t new_capacity = (old_capacity == 0) ? (INITIAL_POOL_SIZE + 1) : (old_capacity * 2);

    Node *new_data = (Node *)realloc(lst->data, new_capacity * sizeof(Node));
    if (new_data == NULL) {
        exit(-1);
    }
...
void CreateList(List *lst) {
    ...
    lst->data = (Node *)malloc((INITIAL_POOL_SIZE + 1) * sizeof(Node));
    if (lst->data == NULL) {
        exit(-1);
    }

    lst->size = 0;
    lst->capacity = INITIAL_POOL_SIZE + 1;

    for (int i = 0; i < INITIAL_POOL_SIZE - 1; i++) {
        lst->data[i].next = i + 1;
    }
    lst->data[INITIAL_POOL_SIZE - 1].next = NULL_INDEX;

    lst->head = INITIAL_POOL_SIZE;
    lst->data[lst->head].next = lst->head;
    lst->top = 0;
}
...
Iterator ListBegin(List *lst) {
    return (Iterator){lst->data[lst->head].next, lst};
}

bool IteratorIsTerminator(Iterator iter) {
    return iter.node == iter.list->head;
}
...
void InsertAfter(Iterator iter, int value) {
    ...
    if (iter.list->top == NULL_INDEX) {
        GrowPool(iter.list);
    }

    int new_node = iter.list->top;
    iter.list->top = iter.list->data[new_node].next;
...
void InsertBegin(List *lst, int value) {
    ...
    int new_node = lst->top;
    lst->top = lst->data[new_node].next;

    lst->data[new_node].value = value;
    lst->data[new_node].next = lst->data[lst->head].next;
    lst->data[lst->head].next = new_node;
    lst->size++;
}
...
void IteratorDeleteNext(Iterator iter) {
    ...
    if (delete_node == iter.list->head) {
        return;
    }

    iter.list->data[iter.node].next = iter.list->data[delete_node].next;
    iter.list->data[delete_node].next = iter.list->top;
    iter.list->top = delete_node;
    iter.list->size--;
}
...
void ListDeleteBegin(List *lst) {
    ...
    int delete_node = lst->data[lst->head].next;
    lst->data[lst->head].next = lst->data[delete_node].next;
    lst->data[delete_node].next = lst->top;
    lst->top = delete_node;
    lst->size--;
}
